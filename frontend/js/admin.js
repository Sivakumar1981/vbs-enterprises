/* ====================================================
   VBS Enterprises — Admin Panel JS
   ==================================================== */
...
// Helper: get correct image URL (Cloudinary or local)
...
function showTab(tab) {
  ...
  const titles = { dashboard: ['Dashboard', 'Overview'], products: ['Products', 'Manage your products'], orders: ['Orders', 'Manage customer orders'] };
  ...
  try {
    ...
    if (ordersData.success) {
      ...
      if (!ordersData.orders.length) {
        div.innerHTML = `<div class="empty-state"><div class="big-icon">📋</div><h3>No orders yet</h3></div>`;
      } else {
        div.innerHTML = `<div class="orders-list">${ordersData.orders.map(orderCardHTML).join('')}</div>`;
      }
    }
    ...
    <tr>
      <td>
        <div class="prod-img-cell">
          ${url
            ? `<img src="${url}" alt="${p.name}" style="width:60px;height:60px;object-fit:cover;border-radius:8px"/>`
            : `<span style="font-size:2rem">${p.category === 'oil' ? '🫙' : '👘'}</span>`
          }
        </div>
      </td>
      ...
      <td><span class="stock-chip ${stockClass}">${stockLabel}</span></td>
...
async function saveProduct() {
  ...
  formData.append('unit', unit || (cat === 'oil' ? 'per litre' : 'per piece'));
...
function editProduct(id) {
  ...
  // Live preview when choosing new image
...
async function toggleProduct(id, active) {
  ...
  const res = await fetch(`${API}/products/${id}`, { method: 'PUT', headers: { Authorization: `Bearer ${token}` }, body: formData });
  const data = await res.json();
  if (data.success) { showToast('✅', active ? 'Product visible' : 'Product hidden'); loadAdminProducts(); }
}

function confirmDeleteProduct(id, name) {
  showConfirm(`Delete "${name}"?`, 'This will permanently remove the product and its image.', async () => {
    try {
...
async function loadOrders() {
  ...
  try {
    ...
    if (!data.orders.length) {
      div.innerHTML = `<div class="empty-state"><div class="big-icon">📋</div><h3>No orders found</h3></div>`;
      return;
    }
    div.innerHTML = `<div class="orders-list">${data.orders.map(o => orderCardHTML(o, true)).join('')}</div>`;
  } catch (err) { showToast('❌', err.message); }
}
...
function orderCardHTML(o, isAdmin = false) {
  ...
  <div class="order-card">
    ...
    <div class="order-items-wrap">
      ${o.items.map(i => `<span class="order-item-chip">${i.category === 'oil' ? '🫙' : '👘'} ${i.name} ×${i.quantity}</span>`).join('')}
    </div>
